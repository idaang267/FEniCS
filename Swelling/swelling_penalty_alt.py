...
#------------------------------------------------------------------------------
mesh = UnitCubeMesh(8, 2, 8)                # Unit Cube
...
# A large penalty parameter deteriorates the problem conditioning, solving time
# will drastically increase and the problem can fail
pen = Constant(1e4)
...
while (steps < tot_steps):
    ...
    p.assign(-project(P(u, mu)[2, 2], V0))  # Project gap
    ...
    if steps > t_indent_steps:
        ...
        F2 = pen*dot(v_u[1], ppos(u[1]-indenter))*ds(1)
        # Full weak form
        WF = F0 + F1 + F2
        problem = NonlinearVariationalProblem(WF, w, bc, J=Jacobian)
        solver_problem = NonlinearVariationalSolver(problem)
