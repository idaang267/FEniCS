...
userpar.add("t_c_steps", 3)
userpar.add("eq_steps2", 60)
userpar.add("c_exp", 1.6)
userpar.add('stable', 5E4)
userpar.parse()

# Parse from command line if given
parPro = 1                      # 1: can't use mpirun
stable = userpar["stable"]
tol = 1E-12
...
savedir = name + sim_param1 + sim_param2 + sim_param3 + sim_param5 + "/"
...
# Define Taylor-Hood Elements XXX Stabilized elements now
...
FStab = - stable*dot(grad(mu-mu0),grad(v_mu))*dx
...
while (steps < tot_steps):
    ...
    if parPro == 1:
        ...
        for a, b in zip(cor, mu_POI):
            if a[2] == 0.0:
                if a[0] == 0.0 or a[0] == 0.1 or a[0] == 0.2 or a[0] == 0.3 or a[0] == 0.4 or a[0] == 0.5 or a[0] == 0.6 or a[0] == 0.7 or a[0] == 0.8 or a[0] == 0.85 or a[0] == 0.9 or a[0] == 0.95 or a[0] == 1.0:
                    #print(a, b)
                    ax = a[0]
                    mu_arr.append(ax)
                    mu_arr.append(b)
