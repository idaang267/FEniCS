...
savedir = name + sim_param1 + sim_param2 + sim_param3 + sim_param4 + "/"
...
#------------------------------------------------------------------------------
# Mesh Resolution
inPlaneRes = 15                  # Along top surface (without refinement)
...
# sym_x.mark(boundaries, 1)
# sym_z.mark(boundaries, 1)
# file_results = XDMFFile(savedir + "/" + "boundaries.xdmf")
...
while (steps < tot_steps):
    ...
    '''
    # Extract displacement values for the points of interest
    u_POI = u.vector()[POI]
    # Create displacement array
    u_arr = []
    # Create displacement dictionary to store coordinate and displacement array
    u_dict = {}
    #
    for a, b in zip(cor, u_POI):
        if a[1] == 0.5:
            if a[0] >= 0.0:
                ...
                ax =  a[0]
                u_arr.append(ax)
                # Append displacement value (scaled by stretch)
                u_arr.append(b)
                # Add to dictionary for the specific time step
                u_dict[t] = u_arr
    ...
    if MPI.rank(MPI.comm_world) == 0:
        # Save information to text file for post-processing
