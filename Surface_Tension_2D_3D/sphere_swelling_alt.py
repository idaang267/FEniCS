...
class InitialConditions(UserExpression):
    def eval(self, values, x):
        ...
        values[3] = chem_ini #ln((l0**3-1)/l0**3) + 1/l0**3 + chi/l0**6 + n*(1/l0-1/l0**3)
...
tol = 1E-12
lm_surf = userpar["lm_surf"]    # Area modulus
mu_surf = userpar["mu_surf"]    # Surface shear modulus
...
st_steps = 0                    # Surface parameter counter (updated within loop)
c_steps = 0                     # Chemical step counter (updated within loop)
t_st_steps = userpar["t_st_steps"]# Total surface parameter (gamma) steps
...
tot_steps = t_st_steps + eq_steps1 + t_c_steps + eq_steps2
...
# Combined boundary conditions: displacement and chemical potential
...
surface_energy_density = 0.5*lm_surf*(ln(Jsurf))**2 + 0.5*mu_surf*(tr(Fsurf) - 2 - 2*ln(Jsurf))
...
data_steps = np.zeros((tot_steps, 5))
...
while (steps < tot_steps):
    ...
    crit1 = t_st_steps + eq_steps1
    crit2 = t_st_steps + eq_steps1 + t_c_steps
    ...
    print("Displacement: " + str(u_POI[0]))
    data_plot[steps] = np.array([t, steps, u_POI[0]+1])

    # Save data to plot
    data_steps[steps] = np.array([t, steps, mu_surf, lm_surf, chem_val])
