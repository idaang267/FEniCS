...
\end{align*}
Changes to relieve the residual stresses seen in the plane stress model:
\begin{equation}
\widetilde{W} (\mathbf{F}, \alpha) = a (\alpha) \frac{\mu}{2} (I_1 - 3 - 2 \ln J) - b(\alpha) p (J-1) - (\alpha^2 + k_\ell) \frac{p^2}{2 \lambda} 
\end{equation}
Where we know $k_\ell$ is a modeling parameter, so we can list the final energy functional as: 
\begin{equation}
\widetilde{W} (\mathbf{F}, \alpha) = a (\alpha) \frac{\mu}{2} (I_1 - 3 - 2 \ln J) - b(\alpha) p (J-1) - \alpha^2 \frac{p^2}{2 \lambda} 
\end{equation}
