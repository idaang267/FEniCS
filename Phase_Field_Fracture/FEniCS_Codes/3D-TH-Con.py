...
simulation_params = "Nx_%.0f_Ny_%.0f_Nz_%.0f_ellx_%.0f_at_%.2f" % (Nx, Ny, Nz, ell_multi, alpha_t)
...
class back_boundary(SubDomain):
    def inside(self, x, on_boundary):
        return on_boundary and near(x[2], 0, 0.1*hsize)

class front_boundary(SubDomain):
    def inside(self, x, on_boundary):
        return on_boundary and near(x[2], W, 0.1*hsize)

class pin_point(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[0], L/2, hsize) and near(x[1], H/2, 0.1*hsize) and near(x[2], W/2, hsize)
...
def P(u, alpha):
    return a(alpha)*mu*(F - inv(F.T)) - d_crit(alpha,alpha_t)*b(alpha)*p*J*inv(F.T)
...
elastic_energy    = (a(alpha)+k_ell)*(mu/2.0)*(Ic-3.0-2.0*ln(J))*dx \
                    - d_crit(alpha,alpha_t)*b(alpha)*p*(J-1.0)*dx \
                    - 1./(2.*kappa)*d_crit(alpha,alpha_t)*p**2*dx
