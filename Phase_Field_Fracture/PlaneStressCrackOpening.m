clear all 
close all

% Theta (rad) 
th = linspace(-pi, pi, 200);
% h0 is the height 
h0 = 1.5;
% Amount of displacement 
Delta = 1.0; 
% stretch in direction of displacement 
lambda_a = 1 + Delta/h0;

% Known a for specific geometry 
ac = 2*sqrt(h0/pi)*(lambda_a - inv(lambda_a));
a = [ac];
% Unknown amplitudes
c = [1.55 1.15];

for ii = 1:length(c)
    y2(ii,:) = (1/a)*sin(th./2);
    y1(ii,:) = 2*c(ii)*y2(ii,:).^2*(cos(th)/(1-cos(th)));    
end 

%% Plotting
% y1_1 = Compiled.y1_norm + .005;
y1_1 = TestS1.y1_norm;
y2_1 = TestS1.y2_norm;
y1_2 = TestS1.y1_norm1;
y2_2 = TestS1.y2_norm1;
y1_3 = TestS1.y1_norm2;
y2_3 = TestS1.y2_norm2;
y1_4 = TestS1.y1_norm3;
y2_4 = TestS1.y2_norm3;
y1_5 = TestS1.y1_norm4;
y2_5 = TestS1.y2_norm4;

y1_1 = CompiledS2.y1_norm;
y2_1 = CompiledS2.y2_norm;
y1_2 = CompiledS2.y1_norm1;
y2_2 = CompiledS2.y2_norm1;
y1_3 = CompiledS2.y1_norm2;
y2_3 = CompiledS2.y2_norm2;
y1_4 = CompiledS2.y1_norm3;
y2_4 = CompiledS2.y2_norm3;

y1_1 = CompiledS1.y1_norm;
y2_1 = CompiledS1.y2_norm;
y1_2 = CompiledS1.y1_norm1;
y2_2 = CompiledS1.y2_norm1;
y1_3 = CompiledS1.y1_norm2;
y2_3 = CompiledS1.y2_norm2;
y1_4 = CompiledS1.y1_norm3;
y2_4 = CompiledS1.y2_norm3;

figure(1)
hold on 
plot(y1(1,:), y2(1,:), 'LineWidth', 2.5)
plot(y1(2,:), y2(2,:), 'LineWidth', 2.5)

h1 = plot(y1_1, y2_1, 'LineWidth', 2.5);
h2 = plot(y1_2, y2_2, 'LineWidth', 2.5);
h3 = plot(y1_3, y2_3, 'LineWidth', 2.5);
h4 = plot(y1_4, y2_4, 'LineWidth', 2.5);
h5 = plot(y1_5, y2_5, 'LineWidth', 2.5);

title(leg, "$No Phase-Field$", 'Interpreter', 'Latex');
leg = legend([h1, h2, h3],["(1-\alpha)^3", "(1-\alpha)^4", "(1-\alpha)^6"], 'Location', 'Best');
leg = legend([h1, h2, h3, h4],["Discrete Stabilized", "Phase-Field Stabilized", "Displacement", "Taylor-Hood"], 'Location', 'Best');

ax = gca;
xlabel("$y_1/a^{2n}$", 'Interpreter', 'LaTeX')
ylabel("$y_2/a^{2n}$", 'Interpreter', 'LaTeX')
xlim([-0.1 0])
ylim([0 0.4])
ax.FontSize = 15; 
set(gca, 'XScale', 'log')
set(gca, 'YScale', 'log')

saveas(gcf, 'PlaneStressCrackOpening.eps', 'epsc')