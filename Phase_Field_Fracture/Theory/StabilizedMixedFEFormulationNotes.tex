...
where 
\begin{align*}
\sqrt{b (\alpha)} = \sqrt{a^3(\alpha)} = (1 - \alpha)^3
\end{align*}
...
\begin{split}
p &= - \sqrt{b (\alpha)} \kappa \left(J-1\right) 
\end{split}
\end{align}
Giving us Eq. 25 in the 2020 Li and Bouklas paper where $\kappa$ is the bulk modulus 
\begin{equation}
\mathcal{E}_{\ell}\left(\boldsymbol{u}, p, \Lambda, \alpha\right) = \mathcal{E}_{\ell} (\boldsymbol{u}, \alpha ) + \int_{\Omega} \frac{p^{2}}{2 \kappa} d \Omega +\int_{\Omega} \Lambda(p + \sqrt{b(\alpha)} \kappa (J-1)) d \Omega
\end{equation}
...
\begin{align}
\begin{split}
\mathcal{E}_{\ell}\left(\boldsymbol{u}, p, \alpha\right) &= \mathcal{E}_{\ell} (\boldsymbol{u}, \alpha ) + \int_{\Omega} \frac{p^{2}}{2 \kappa} d \Omega +\int_{\Omega} - \frac{p}{\kappa} (p+\sqrt{b(\alpha)} \kappa (J-1)) d \Omega \\
		&= \mathcal{E}_{\ell} (\boldsymbol{u}, \alpha ) + \int_{\Omega} \frac{p^{2}}{2 \kappa} d \Omega -\int_{\Omega} \frac{p^2}{\kappa} d \Omega - \int_{\Omega} \frac{p}{\kappa} \sqrt{b(\alpha)} \kappa (J-1)) d \Omega \\
\mathcal{E}_{\ell}\left(\boldsymbol{u}, p, \alpha\right) &= \mathcal{E}_{\ell} (\boldsymbol{u}, \alpha ) - \int_{\Omega} \frac{p^{2}}{2 \kappa} d \Omega - \int_{\Omega} \sqrt{b(\alpha)} p(J-1)) d \Omega
\end{split}
\end{align}
Substitute in $\mathcal{E}_{\ell} (\boldsymbol{u}, \alpha )$ and substitute Eq. \ref{StrainEnergyNH}
\begin{align*}
\begin{split}
\mathcal{E}_{\ell}\left(\boldsymbol{u}, p, \alpha\right) &= \int_{\Omega} a(\alpha) \mathcal{W}(\mathbf{F}) d \Omega+\frac{G_{c}}{c_{w}} \int_{\Omega} \bigg(\frac{w(\alpha)}{\ell}+\ell\|\nabla \alpha\|^{2} \bigg) d \Omega- \int_{\Omega} \frac{p^{2}}{2 \kappa} d \Omega - \int_{\Omega} \sqrt{b(\alpha)} p(J-1) d \Omega 
\end{split}
\end{align*}
...
\begin{align*}
- \frac{\varpi h^2}{2\mu} \sqrt{b(\alpha)} \sum_{e=1}^{n_{el}}\int_{\Omega^{e}} J \mathbf{C}^{-1} : \big( \nabla p \cdot \nabla q \big) \,dV = 0 
\end{align*}

\subsection{Summary}
Therefore the modified strain energy functional can be written as follows: 
\begin{align*}
\widetilde{W} (\mathbf{F}, p, \alpha) 
		&= a(\alpha) \mathcal{W} (\mathbf{F}) - \sqrt{b (\alpha)} p (J-1) - \frac{p^2}{2 \kappa} \\
\widetilde{W} (\mathbf{F}, \alpha) 
		&= a(\alpha) \mathcal{W} (\mathbf{F}) + \frac{\kappa}{2} b (\alpha) (J-1)^2
\end{align*}
In the code, we have a small number for numerical purposes 
\begin{align*}
\widetilde{W} (\mathbf{F}, \alpha) &= \big( a(\alpha) + k_\ell) \frac{\mu}{2} (I_c - 3 - 2 \ln J) - \sqrt{b(\alpha)} p (J-1) - \frac{p^2}{2 \kappa}
\end{align*}
...
\mathbf{P} &= \frac{\partial \widetilde{\mathcal{W}}(\mathbf{F},\alpha)}{\partial \mathbf{F}} \\
		 &= \frac{\partial}{\partial \mathbf{F}} \bigg[ a(\alpha)\mathcal{W}(\mathbf{F}) + b(\alpha) \frac{\kappa}{2} \left(J-1\right)^2 \bigg] \\
		 &= a(\alpha)\frac{\partial \mathcal{W}(\mathbf{F})}{\partial \mathbf{F}} + b(\alpha) \frac{1}{2} \kappa \frac{\partial (J-1)^2}{\partial \mathbf{F}}  \\
		 &= a(\alpha)\frac{\partial \mathcal{W}(\mathbf{F})}{\partial \mathbf{F}} + b(\alpha)\kappa\left(J-1\right)\frac{\partial J}{\partial \mathbf{F}} \quad \text{where } \frac{\partial J}{\partial \mathbf{F}} = J \mathbf{F}^{-T} \\
		 &= a(\alpha)\frac{\partial \mathcal{W}(\mathbf{F})}{\partial \mathbf{F}} + b(\alpha)\kappa (J-1) J \mathbf{F}^{-T} \quad \text{substituting in pressure equation} \\
\mathbf{P} &= a(\alpha)\frac{\partial \mathcal{W}(\mathbf{F})}{\partial \mathbf{F}} - b(\alpha) p J \mathbf{F}^{-T}
\end{split}
\end{align}
...
%\end{cases}
%\end{align*}
Consider the modified strain energy 
\begin{align*}
\widetilde{W} (\mathbf{F}, \alpha) 
	&= a(\alpha) \frac{\mu}{2} (I_c - 3 - 2 \ln J) - \sqrt{b(\alpha)} p (J-1) - \frac{p^2}{2 \kappa} \quad \text{where } p = - \sqrt{b(\alpha)} \kappa (J-1) \\
\widetilde{W} (\mathbf{F}, \alpha) 
	&= a(\alpha) \frac{\mu}{2} (I_c - 3 - 2 \ln J) + \frac{\kappa}{2} b(\alpha) (J-1)^2
\end{align*}
We can rewrite the last term with regards to stretches
\begin{align*}
\widetilde{W} (\mathbf{F}, \alpha) &= a(\alpha) \frac{\mu}{2} \sum_{i = 1}^3 (\lambda_i^2 - 1 - 2 \ln \lambda_i) + \frac{\kappa}{2} b(\alpha) (J-1)^2
\end{align*}
...
\begin{align*}
\widetilde{W}_{act} (\mathbf{F}, \alpha) &= a(\alpha) \frac{\mu}{2} \sum_{i = 1}^3 ( (\lambda_i^+)^2 - 1 - 2 \ln \lambda_i^+) + \frac{\kappa}{2} b(\alpha) (J^+ -1)^2 \\
\widetilde{W}_{pas} (\mathbf{F}, \alpha) &= \frac{\mu}{2} \sum_{i = 1}^3 ( (\lambda_i^-)^2 - 1 - 2 \ln \lambda_i^-) + \frac{\kappa}{2} (J^- -1)^2
\end{align*}
...
\text{For } J > 1: & \\
& \widetilde{W}_{act} (\mathbf{F}, \alpha) = a(\alpha) \frac{\mu}{2} \sum_{i = 1}^3 ( (\lambda_i^+)^2 - 1 - 2 \ln \lambda_i^+) + \frac{\kappa}{2} b(\alpha) (J-1)^2 \\
& \widetilde{W}_{pas} (\mathbf{F}, \alpha) = \frac{\mu}{2} \sum_{i = 1}^3 ( (\lambda_i^-)^2 - 1 - 2 \ln \lambda_i^-) \\
\text{For } J \leq 1: \\
& \widetilde{W}_{act} (\mathbf{F}, \alpha) = a(\alpha) \frac{\mu}{2} \sum_{i = 1}^3 ( (\lambda_i^+)^2 - 1 - 2 \ln \lambda_i^+) \\
& \widetilde{W}_{pas} (\mathbf{F}, \alpha) = \frac{\mu}{2} \sum_{i = 1}^3 ( (\lambda_i^-)^2 - 1 - 2 \ln \lambda_i^-) + \frac{\kappa}{2} b(\alpha) (J -1)^2 
\end{split}
\end{align}
...
& \widetilde{W}_{act} (\mathbf{F}, \alpha) = a(\alpha) \frac{\mu}{2} \sum_{i = 1}^3 ( \lambda_i^2 - 1 - 2 \ln \lambda_i) \\
& \widetilde{W}_{pas} (\mathbf{F}, \alpha) = \frac{\kappa}{2} (J -1)^2 
\end{align*}
all other cases:
\begin{align*}
\text{For } J > 1,\, \lambda_i < 1: & \\
& \widetilde{W}_{act} (\mathbf{F}, \alpha) =  \frac{\kappa}{2} b(\alpha) (J-1)^2 \\
& \widetilde{W}_{pas} (\mathbf{F}, \alpha) = \frac{\mu}{2} \sum_{i = 1}^3 ( \lambda_i^2 - 1 - 2 \ln \lambda_i) \\
\text{For } J \leq 1,\, \lambda_i < 1: \\
& \widetilde{W}_{act} (\mathbf{F}, \alpha) = 0 \\
& \widetilde{W}_{pas} (\mathbf{F}, \alpha) = \frac{\mu}{2} \sum_{i = 1}^3 ( \lambda_i^2 - 1 - 2 \ln \lambda_i) + \frac{\kappa}{2} (J-1)^2 
\end{align*}
These can be concisely summarized with the following expressions, where the active and passive parts of the strain energy are
\begin{align*}
\begin{split}
\widetilde{W}_\text{act}\left(\mathbf{F},\alpha\right)
&= a(\alpha)\frac{\mu}{2}\sum_{i=1}^{3}H(\lambda_i-1)\left(\lambda_i^2-1-2\ln\lambda_i\right) + b(\alpha)H(J-1)\frac{1}{2}\kappa\left(J-1\right)^2, \\
\widetilde{W}_\text{pas}\left(\mathbf{F},\alpha\right)
&= \frac{\mu}{2}\sum_{i=1}^{3}H(1-\lambda_i)\left(\lambda_i^2-1-2\ln\lambda_i\right) + H(1-J)\frac{1}{2}\kappa\left(J-1\right)^2
\end{split}
\end{align*}

\subsection{Compute the principal stretches $\lambda_i$} 
...
%===============================
%===============================
\subsection{Obtaining the Critical Stretch}
